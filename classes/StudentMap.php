...
class StudentMap extends BaseMap {
  ...
  public function findAll($ofset = 0, $limit = 30)
  {
    $res = $this->db->query("SELECT user.user_id, CONCAT(user.lastname,' ', user.firstname, ' ', user.patronymic) AS fio, user.birthday, "
      . "gender.name AS gender, gruppa.name AS gruppa, student.num_zach AS num_zach, role.name AS role "
      . "FROM user "
  ...
  {
    if ($id) {
      $res = $this->db->query("SELECT student.user_id, gruppa.name AS gruppa, num_zach FROM student "
