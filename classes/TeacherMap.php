...
class TeacherMap extends BaseMap
{
  ...
  public function findAll($ofset = 0, $limit = 30)
  {
    $res = $this->db->query("SELECT user.user_id, CONCAT(user.lastname,' ', user.firstname, ' ', user.patronymic) AS fio, user.birthday, "
      . " gender.name AS gender, otdel.name AS otdel, role.name AS role FROM user INNER JOIN teacher ON user.user_id=teacher.user_id "
      . "INNER JOIN gender ON user.gender_id=gender.gender_id INNER JOIN otdel ON teacher.otdel_id=otdel.otdel_id"
