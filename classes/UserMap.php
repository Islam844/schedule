...
class UserMap extends BaseMap {
  ...
  private function existLoginById($id,$login)
  {
    $id = $this->db->quote($id);
    $res = $this->db->query("SELECT login FROM user WHERE user_id = $id");
    if($res->fetchColumn() == $login)
    {
      return true;
    }
    return false;
  ...
  public function findProfileById($id = null)
  {
    if ($id) {
      $res = $this->db->query("SELECT user.user_id, CONCAT(user.lastname,' ', user.firstname, ' ', user.patronymic) AS fio,"
      . " user.login, user.birthday, gender.name AS gender, role.name AS role, user.active FROM user "
      . "INNER JOIN gender ON user.gender_id=gender.gender_id INNER JOIN role ON user.role_id=role.role_id WHERE user.user_id = $id");
  ...
  */
  ...
  public function save(User $user)
  {
    if ($user->user_id == 0) {
      ...
      else if($this->existLoginById($user->user_id, $user->login))
      {
        return $this->update($user);
      }
    }
