...
class LessonPlanMap extends BaseMap {
  ...
  public function arrPlanByTeacherId($id = null)
  {
    ...
    . " FROM lesson_plan INNER JOIN gruppa ON lesson_plan.gruppa_id=gruppa.gruppa_id "
    . "INNER JOIN subject ON lesson_plan.subject_id=subject.subject_id "
    . "WHERE lesson_plan.user_id=$id ORDER BY gruppa.name, subject.name");
  ...
  public function findTeachers($ofset = 0, $limit = 30)
  {
    ...
    . " otdel.name AS otdel, COUNT(lesson_plan.subject_id) AS count_plan,SUM(subject.hours) AS sum_hours "
    . "FROM user INNER JOIN teacher ON user.user_id=teacher.user_id INNER JOIN otdel "
    . "ON teacher.otdel_id=otdel.otdel_id LEFT OUTER JOIN lesson_plan ON teacher.user_id=lesson_plan.user_id "
    . "LEFT OUTER JOIN subject ON lesson_plan.subject_id=subject.subject_id GROUP BY user.user_id "
    . "LIMIT $ofset, $limit");
    return $res->fetchAll(PDO::FETCH_OBJ);
  }
